...
// 设置登陆网络环境
KENETSDKSHARED_EXPORT void KENet_SetNetworkParam(int sID,NET_PARAM *netParam);


//开始监听连接,用于设备主动连接的情况
KENETSDKSHARED_EXPORT int KENet_StartListen(int sID,fNewConnection newConnect, void * user);

//链接设备,return connect handle,usually the connectHandle is equal to sID.
KENETSDKSHARED_EXPORT int KENet_Connect(
        int sID,const char * addr,int port,fDisConnect cbDisConnect, void * user);

// 设置断线重连成功回调函数，设置后SDK内部断线自动重连
KENETSDKSHARED_EXPORT int KENet_SetAutoReconnect(int connectHandle,fHaveReConnect cbAutoConnect,void *user);
...
//        int connectHandle,const char *userName,const char *password,
//        DeviceTreeNode * info,int *error = 0);
...
KENETSDKSHARED_EXPORT int KENet_StartRealPlay(
        ...
        fRealPlayDisConnect cbDisconnect, void * dwUser, int dwWaitTime = 10000);
...
KENETSDKSHARED_EXPORT int KENet_QuickQueryRecordFile(
        ...
        int maxlen, int *filecount, int waittime=1000);
